...
export default function DashboardUserPage() {
  ...
  // === تصحيح: إعادة الكود الكامل لـ useEffect الخاص بالنقر الخارجي ===
  useEffect(() => {
    // تعريف الدالة التي سيتم استدعاؤها عند النقر
    function handleClickOutside(event: MouseEvent) {
      // التحقق إذا كان الشريط مفتوحاً، والمرجع موجوداً، والنقر حدث خارج عنصر الشريط
      if (isOpen && sidebarRef.current && !sidebarRef.current.contains(event.target as Node)) {
        setIsOpen(false); // إغلاق الشريط
      }
    }
    // إضافة مستمع الحدث عند تحميل المكون أو تغير حالة الشريط
    document.addEventListener('mousedown', handleClickOutside);
    // إزالة مستمع الحدث عند إزالة المكون أو تغير حالة الشريط (لتنظيف الذاكرة)
    return () => {
  ...
  // useEffect لتحديث الطالب المختار (يبقى كما هو)
  useEffect(() => {
